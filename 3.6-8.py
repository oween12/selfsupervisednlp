...
def evaluate_model(model, dataloader, device):
    ...
    for batch in dataloader:
        ...
        dev_accuracy.add_batch(predictions=predictions, references=batch['labels'].to("cuda"))
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, device, lr):
    ...
    for epoch in range(num_epochs):
        ...
        for i, batch in enumerate(train_dataloader):
            ...
            output = pretrained_model(input_ids, attention_mask)
            predictions = torch.max(output.logits, dim=1)
            model_loss = loss(output.logits, batch["labels"].to("cuda"))
            model_loss.backward()
            optimizer.step()
            lr_scheduler.step()
            optimizer.zero_grad()
            ...
            # update metrics
            train_accuracy.add_batch(predictions=predictions, references=batch['labels'].to("cuda"))

        # print evaluation metrics
        print(f" ===> Epoch {epoch + 1}")
        cur_train_accuracy = train_accuracy.compute()
        print(f" - Average training metrics: accuracy={cur_train_accuracy}")
        train_acc[epoch] = cur_train_accuracy['accuracy']

        # normally, validation would be more useful when training for many epochs
        val_accuracy = evaluate_model(mymodel, validation_dataloader, device)
        print(f" - Average validation metrics: accuracy={val_accuracy}")
        val_acc[epoch] = val_accuracy['accuracy']
...
if __name__ == "__main__":
    ...
    plt.ylabel("Training and Val Accuracy %")
    plt.xlabel("Epochs")
    plt.legend(['Train', 'Validation'])
    plt.title("Train and Val Accuracy % vs Epochs")
    plt.savefig(str(args.model) + "-" + str(args.lr) + "-" + str(args.batch_size) + '-q7-plot.png')
    plt.show()
